...
```{r, include=FALSE}
# Read in Your Data 
cces_2016 <- read.csv("~/Documents/Resurch methods Document/ResurchMethods/CCES.csv")
```
...
```{r}
ggplot(data = cces_2016, aes(x = female, y = ideology)) + 
  geom_boxplot()  + 
  labs(title = "")
...
cces_2016 %>% 
  ggplot(aes(x = female, y = ideology)) + 
  geom_boxplot()  + 
  labs(title = "")
```
...
```{r, echo=FALSE}
pander(prop.table(table(d1$gender, d1$partyid)))
```
...
ggplot(data = d1, aes(x = woman, y = partyid)) +
  geom_boxplot()  +
  theme(legend.position = "right") +
  labs(title = "")
ggplot(data = d1, aes(x = woman, y = ideology)) +
  geom_boxplot()  +
  theme(legend.position = "right") +
  labs(title = "")
```
...
```
```{r}
t.test(partyid~gender, data=cces_2016)
```
