...
```{r}
mean(cces_2016$partyid, na.rm = TRUE)
```

What is the median of partyid?

```{r}
median(cces_2016$partyid, na.rm = TRUE)
```
...
```{r, echo=FALSE}
pander(prop.table(table(cces_2016$gender, cces_2016$ideology)))
```
...
party <- ggplot(data = cces_2016, aes(x = gender, y = partyid)) +
  geom_boxplot()  +
