...
Say we wanted to understand the relationship between the strength of an individual ($X$) and how far they can throw a ball  ($Y$).

We have options on how we believe balls fly. Do we think it moves in an arc? Does it immediately drop based on gravity? This choice is our specification of $f$ or the functional of ball trajectory. 

Our choice in $f$ decides where we think the ball ends up. In other words, our choice of $f$ matters a lot!
...
- We could specify $\hat{f}$ to get a better picture of $Y$, therefore, that error is considered reducible.
...
- We tradeoff between *interpretability* and *accuracy.* Regression, which we will spend the bulk of this week on, is highly interpretable. You will see we have clear inputs and outputs, and our $\hat{f}$ will be clearly defined. In situations of machine learning such as ensemble methods (random forests and gradient boosting), the predictions of our outcomes are highly accurate, but the $\hat{f}$ remains unknown (black box methods).
...
- **Experimental data** controls for external forces by leveraging the benefits of random assignment. This data has randomly assigned treatment and control groups, which alleviates the issues of confounders since the only functional difference between the groups in their assignment of the treatment, and that choice was made at random. 
